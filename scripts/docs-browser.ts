...
const origin = "https://docs.apps.acm.sharkbait.tech";
const evidencePrefix = "";
const secret = JSON.parse(
  execFileSync(
    "oc",
    ["get", "secret", "docs-sender-accounts", "-n", "docs", "-o", "json"],
...
try {
  ...
  assert.equal(await page.title(), "Docs signed by Sharkbait");
  assert.equal(new URL(origin).hostname, "docs.apps.acm.sharkbait.tech");
  const passwordInput = page.getByLabel("Bundle password", { exact: true });
  ...
  await page
    .getByLabel("Documents", { exact: true })
    .setInputFiles({
      name: "selection-check.txt",
      mimeType: "text/plain",
      buffer: Buffer.from("selection check"),
    });
  await page.getByRole("button", { name: "Clear selected documents" }).click();
  assert.equal(
    await page
      .getByLabel("Documents", { exact: true })
  ...
  results.brandAndInputControls = true;
  ...
  const backupJob = "docs-backup-check-" + Date.now().toString(36);
  execFileSync(
    "oc",
    [
      "create",
      "job",
      backupJob,
      "--from=cronjob/docs-metadata-backup",
      "-n",
      "docs",
    ],
    { stdio: "pipe" },
  ...
  results.responsiveWidths = [390, 768, 1280, 1920];
  ...
  await page.screenshot({
    path: `evidence/docs/${evidencePrefix}sender.png`,
  });
  ...
  await writeFile(
    `evidence/docs/${evidencePrefix}browser-workflow.json`,
